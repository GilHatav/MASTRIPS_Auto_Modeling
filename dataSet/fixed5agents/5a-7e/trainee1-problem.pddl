...
(define (problem gym-training) (:domain gym-palnner)
    ...
    (:init
		...
		(want pers1 mach_PullUps)
		(want pers1 mach_Back_TBarRow)
		(want pers1 mach_Deadlift)
		(want pers1 mach_Arms_SeatedInclineDumbbellCurl)
		(want pers1 mach_Squat)
    )
    (:goal
        (and
            (done pers1 mach_PushPress)
			(done pers1 mach_PushUps)
			(done pers1 mach_PullUps)
			(done pers1 mach_Back_TBarRow)
			(done pers1 mach_Deadlift)
			(done pers1 mach_Arms_SeatedInclineDumbbellCurl)
			(done pers1 mach_Squat)
        )
    )
)
